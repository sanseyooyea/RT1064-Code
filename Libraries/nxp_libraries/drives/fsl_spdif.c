...
status_t SPDIF_TransferReceiveNonBlocking(SPDIF_Type *base, spdif_handle_t *handle, spdif_transfer_t *xfer) {
    ...
    /* Enable interrupt */
    SPDIF_EnableInterrupts(base, (uint32_t)
    kSPDIF_UChannelReceiveRegisterFull |
    (uint32_t)
    kSPDIF_QChannelReceiveRegisterFull | (uint32_t)
    kSPDIF_RxFIFOFull |
    (uint32_t)
...
void SPDIF_TransferAbortReceive(SPDIF_Type *base, spdif_handle_t *handle) {
    ...
    /* Disable interrupt */
    SPDIF_DisableInterrupts(base, (uint32_t)
    kSPDIF_UChannelReceiveRegisterFull |
    (uint32_t)
    kSPDIF_QChannelReceiveRegisterFull | (uint32_t)
    kSPDIF_RxFIFOFull |
...
void SPDIF_TransferTxHandleIRQ(SPDIF_Type *base, spdif_handle_t *handle) {
    ...
    kSPDIF_TxFIFOEmpty) != 0x00U))
    {
        ...
        handle->spdifQueue[handle->queueDriver].dataSize -= (uint32_t)
        dataSize * 6U;
...
void SPDIF_TransferRxHandleIRQ(SPDIF_Type *base, spdif_handle_t *handle) {
    ...
    ((base->SIE & (uint32_t)
    kSPDIF_QChannelReceiveRegisterFull) != 0x00U))
    {
    ...
    ((base->SIE & (uint32_t)
    kSPDIF_UChannelReceiveRegisterFull) != 0x00U))
    {
